...
from slowfast.utils.metrics import topks_correct  # optional if needed
from fvcore.common.timer import Timer

def binary_framewise_accuracy(preds, labels, threshold=0.5):
    """
    Compute binary framewise accuracy.
    preds: [B, T] logits or probabilities
    labels: [B, T] ground truth 0/1
    """
...
def binary_framewise_precision(preds, labels, threshold=0.5):
    """
    Compute precision for binary predictions.
    preds: [B, T] logits or probabilities
    labels: [B, T] ground truth 0/1
    """
    preds = (preds > threshold).float()
    ...
    precision = tp / (tp + fp + 1e-8)  # 加个 epsilon 避免除 0
    return precision


logger = logging.get_logger(__name__)
def new_train_epoch(train_loader, model, optimizer,scaler,train_meter, cur_epoch, cfg,    writer=None,):
    """
    Perform the training for one epoch.
    """
    print("Training epoch {}".format(cur_epoch+1))
    model.train()
    ...
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(train_loader):
        ...
        if loss_fun is None:
            with torch.no_grad():
                ...
                # 避免除以0
                pos_weight = torch.tensor(6.36)

                weight = torch.tensor(6.36, device=labels.device)
                print(f"[INFO] Auto-calculated pos_weight: {weight.item():.2f}")
            loss_fun = losses.get_loss_func(cfg.MODEL.LOSS_FUNC)(
                pos_weight=weight.to(labels.device),
                reduction="mean"
            )
        ...
        # Framewise accuracy and precision
        precision = torch.tensor(0.0).to(loss.device)

        with torch.no_grad():
            acc = binary_framewise_accuracy(preds.sigmoid(), labels)
            precision = binary_framewise_precision(preds.sigmoid(), labels)

        if cfg.NUM_GPUS > 1:
            loss, grad_norm, acc, precision = du.all_reduce(
                [loss.detach(), grad_norm, acc.detach(), precision.detach()]
            )

        loss, grad_norm, acc, precision = (
            loss.item(), grad_norm.item(), acc.item(), precision.item()
        )

        train_meter.update_stats(
            acc,  # replaces top1_err
            precision,  # also fill top5_err slot
            loss,
            lr,
            grad_norm,
    ...
    train_meter.reset()
    print("当前显存使用：", torch.cuda.memory_allocated() / 1e6, "MB")
    print("内存占用：", psutil.Process(os.getpid()).memory_info().rss / 1024 ** 2, "MB")
...
def train_epoch(
    ...
    pos_weight = torch.tensor(6.36)
    loss_fun = losses.get_loss_func(cfg.MODEL.LOSS_FUNC)(pos_weight=pos_weight, reduction="mean")
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(train_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                # Compute the errors.
                num_topks_correct = [torch.tensor(0.0).to(preds.device) for _ in (1, 5)]
                top1_err, top5_err = [
                    (1.0 - x / preds.size(0)) * 100.0 for x in num_topks_correct
                ]
                # ✅ 将 top1_err, top5_err 包装为 Tensor

                loss = loss.detach() if not isinstance(loss, torch.Tensor) else loss
                grad_norm = grad_norm.detach() if not isinstance(grad_norm, torch.Tensor) else grad_norm

                top1_err = torch.tensor(top1_err, device=preds.device)
                top5_err = torch.tensor(top5_err, device=preds.device)
    ...
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(val_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                if cfg.NUM_GPUS > 1:
                    acc, precision = du.all_reduce([acc.detach(), precision.detach()])
...
def train(cfg):
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        if cfg.TRAIN.DATASET == "greathits":
            new_train_epoch(
            train_loader,
            model,
            optimizer,
